 ...
 * Share Network Story
 */
function share_story_network_link( $network, $post_id = null, $class = '', $icon = false, $styling = '' ) {
	...
	$networks = array(
		...
		'wa' => array(
			'url'     => 'https://api.whatsapp.com/send?text=' . get_the_title( $post_id ) . ' - ' . $permalink,
			'tooltip' => 'WhatsApp',
			'icon'    => 'whatsapp'
		),
    ...
    <a class="<?php echo esc_attr( trim( "{$network_entry['icon']} {$class}" ) ); ?>"
       href="<?php echo $network_entry['url']; ?>"<?php when_match( $new_window, 'target="_blank"' ); ?> aria-label="<?php echo esc_attr( $network_entry['tooltip'] ); ?>">
		<?php if ( $icon ) : ?>
